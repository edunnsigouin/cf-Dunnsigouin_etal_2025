...
def setup_subplot_xy(subplot_flag, ax, fss, sig, title_text, clevs, cmap, fontsize):
    """ 
    ...
    if (subplot_flag == 0) or (subplot_flag == 2):
        rectangle = plt.Rectangle((-25, 34), 0.25, 0.25, fc='r',ec='r')
    elif (subplot_flag == 1) or (subplot_flag == 3):
...
filename_in_3     = 'fss_xy_tp24_weekly_europe_annual_boxsize_1_leadtime_2_2020-01-02_2022-12-29_0.25x0.25.nc'
...
setup_subplot_xy(1, ax[2], ds2['fss'], sig2, 'c) lead time: 5 days, spatial scale: 25 gridpoints$^{2}$ / 225km$^{2}$', clevs, cmap, fontsize)
setup_subplot_xy(2, ax[1], ds3['fss'], sig3, 'b) lead time: 2 weeks, spatial scale: 1 gridpoints$^{2}$ / 9km$^{2}$', clevs, cmap, fontsize)
