...
init_start        = '2023-08-05'        # initialization date
leadtime_day      = 3                   # lead time in days
...
figname_out = 'xy_' + variable + '_' + grid + '_' + domain + '_init_' + init_start + '_leadtimeday_' + str(leadtime_day) + '_NH_' + str(NH[0]) + '.pdf'

# read in data & take ensemble mean
da    = xr.open_dataset(path_in + filename_in).isel(time=leadtime_day).mean(dim='number')[variable] # lead time indexing starts at day 0 (lead time 1 day)
...
p0 = axes[0].contourf(dim.longitude,dim.latitude,da,levels=clevs,cmap=cmap,extend='both',transform=ccrs.PlateCarree())
...
cb0 = fig.colorbar(p0, ax=axes[0], orientation='vertical',ticks=clevs[::2].astype(int),pad=0.025,aspect=15)
...
#p0.cmap.set_under('white')

p1 = axes[1].contourf(dim.longitude,dim.latitude,da_frac[3,:,:],levels=clevs,cmap=cmap,extend='both',transform=ccrs.PlateCarree())
...
cb1 = fig.colorbar(p1, ax=axes[1], orientation='vertical',ticks=clevs[::2].astype(int),pad=0.025,aspect=15)
cb1.ax.set_title('[' + units + ']',fontsize=fontsize)
cb1.ax.tick_params(labelsize=fontsize,size=0)
#axes[0].set_title('era5 date = ' + date,fontsize=fontsize)

p1.cmap.set_under('red')
