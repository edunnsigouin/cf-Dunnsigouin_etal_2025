...
# -----------------------------
# 2. Create synthetic rainfall with extra structure
# -----------------------------
np.random.seed(42)

# (A) Random noise (increased amplitude)
random_field = 15.0 * np.random.rand(num_lat, num_lon)
...
# (C) Multiple Gaussian "lumps" for localized heavy rain
...
# Combine all components + offset to ensure positive values
rain_data_original = 10 + random_field + wave_pattern + lumps

# -----------------------------
# 3. Apply spatial smoothing (reduced sigma for less blur)
# -----------------------------
sigma = 2.0
rain_data_smoothed = gaussian_filter(rain_data_original, sigma=sigma)

# -----------------------------
# 4. Define a larger, irregular watershed
# -----------------------------
