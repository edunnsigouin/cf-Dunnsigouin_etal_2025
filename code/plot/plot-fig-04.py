...
def setup_subplot(ax, time, box_size, fss_data, sig_data, title_text, clevs, cmap, fontsize):
    ...
    """
    if (title_text == 'c) >90$^{th}$ minus <10$^{th}$ quantile precipitation') or (title_text == 'f) >90$^{th}$ minus <10$^{th}$ quantile temperature'):
    ...
    ax.clabel(contour, clevs, inline=True, fmt='%1.2f', fontsize=fontsize)
    ...
    if (title_text == 'd) >90$^{th}$ quantile temperature') or (title_text == 'e) <10$^{th}$ quantile temperature'): 
    ...
    if (title_text == 'a) >90$^{th}$ quantile precipitation') or (title_text == 'd) >90$^{th}$ quantile temperature'):
...
filename_in_2     = 'fbss_tp24_pval0.1_time_europe_annual_2020-01-02_2022-12-29_0.25x0.25.nc'
filename_in_3     = 'fbss_difference_time_tp24_pval0.9_europe_annual_2020-01-02_2022-12-29_tp24_pval0.1_europe_annual_2020-01-02_2022-12-29_0.25x0.25.nc'
...
filename_in_6     = 'fbss_difference_time_t2m24_pval0.9_europe_annual_2021-01-04_2021-12-30_t2m24_pval0.1_europe_annual_2021-01-04_2021-12-30_0.25x0.25.nc'
...
figsize    = np.array([15,7.5])
fig,ax     = plt.subplots(nrows=2,ncols=3,sharey='row',sharex='col',figsize=(figsize[0],figsize[1]))
ax         = ax.ravel()

#cmap_anom = misc.create_custom_colormap_with_white_center(cmap_anom, clevs_anom)


# A) precipitation
setup_subplot(ax[0], ds1['time'], ds1['box_size'], ds1['fbss'], sig1, 'a) >90$^{th}$ quantile precipitation', clevs, cmap, fontsize)

# B) temperature 
setup_subplot(ax[1], ds2['time'], ds2['box_size'], ds2['fbss'], sig2, 'b) <10$^{th}$ quantile precipitation', clevs, cmap, fontsize)

# C) precip minus temperature 
setup_subplot(ax[2], ds3['time'], ds3['box_size'], ds3['fbss'], sig3, 'c) >90$^{th}$ minus <10$^{th}$ quantile precipitation', clevs_anom, cmap_anom, fontsize)

# D) precip 
setup_subplot(ax[3], ds4['time'], ds4['box_size'], ds4['fbss'], sig4, 'd) >90$^{th}$ quantile temperature', clevs, cmap, fontsize)

# E) temperature 
setup_subplot(ax[4], ds5['time'], ds5['box_size'], ds5['fbss'], sig5, 'e) <10$^{th}$ quantile temperature', clevs, cmap, fontsize)

# F) temperature minus precipitation 
setup_subplot(ax[5], ds6['time'],ds6['box_size'], ds6['fbss'], sig6, 'f) >90$^{th}$ minus <10$^{th}$ quantile temperature', clevs_anom, cmap_anom, fontsize)
