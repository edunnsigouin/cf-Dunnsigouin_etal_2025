...
def setup_subplot(ax, time, box_size, fss_data, sig_data, title_text, clevs, cmap, fontsize):
    ...
    if time.size == 6:
        ...
        if title_text == 'd) <10$^{th}$ quantile weekly-mean precipitation':
            ax.set_xlabel(r'lead time [weeks]',fontsize=fontsize)
    else:
        ax.set_xticklabels(['1', '', '3', '', '5', '', '7', '', '9', '', '11', '', '13', '', '15'], fontsize=fontsize)
        if title_text == 'c) <10$^{th}$ quantile daily-mean precipitation':
    ...
    if (title_text == 'a) >90$^{th}$ quantile daily-mean precipitation') or (title_text == 'c) <10$^{th}$ quantile daily-mean precipitation'):
...
filename_in_4     = 'fbss_tp24_pval0.1_weekly_europe_annual_2021-01-04_2021-12-30.nc'
...
setup_subplot(ax[0], ds1['time'], ds1['box_size'], ds1['fbss'], sig1, 'a) >90$^{th}$ quantile daily-mean precipitation', clevs, cmap1, fontsize)

# B) weekly precipitation 
setup_subplot(ax[1], ds2['time'], ds2['box_size'], ds2['fbss'], sig2, 'b) >90$^{th}$ quantile weekly-mean precipitation', clevs, cmap1, fontsize)

# C) temperature 90th quantile
setup_subplot(ax[2], ds3['time'], ds3['box_size'], ds3['fbss'], sig3, 'c) <10$^{th}$ quantile daily-mean precipitation', clevs, cmap2, fontsize)

# D) weekly temperature
setup_subplot(ax[3], ds4['time'], ds4['box_size'], ds4['fbss'], sig4, 'd) <10$^{th}$ quantile weekly-mean precipitation', clevs, cmap2, fontsize)
