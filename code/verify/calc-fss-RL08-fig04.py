...
space            = 0 # location of observational rain band in domain
Io               = np.zeros([Nx,Ny])
Im               = np.zeros([Nx,Ny,4])
Io[:,space+0]    = 1
Im[:,space+1,0]  = 1
...
for i in range(0,4,1):
    ax.plot(x+1,FSS[x,i],color=colors[i],linewidth=1.5)
    ax.plot(x+1,FSS[x,i],marker='o',markersize=fontsize-5,color=colors[i],label=labels[i])
