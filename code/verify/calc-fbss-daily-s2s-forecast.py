"""
Calculates the fractional brier skill score as a function of lead time and box size 
...
season                   = 'ndjfm'                 # pick forecasts in specific season (djf,mam,jja,son,annual)
...
for grid in grids:
    ...
    print('\ncalculating fbss for variable ' + variable + ' on grid ' + grid)
    ...
    [fbss,fbss_bootstrap] = verify.initialize_score_array('fbss',dim,box_sizes_temp,number_shuffle_bootstrap)
    ...
    for  i, date in enumerate(forecast_dates):
        ...
        filename_verification                           = path_in_verification + 'binary_' + variable + '_' + time_flag + '_' + grid + '_' + date + '.nc'
        filename_forecast                               = path_in_forecast + 'probability_' + variable + '_' + time_flag + '_' + grid + '_' + date + '.nc'
        forecast_error[i, ...], reference_error[i, ...] = verify.calc_forecast_and_reference_error('fbss',filename_verification, filename_forecast, variable, box_sizes_temp, dim, pval)
       
    # calc fbss with bootstraping over all forecasts  
    fbss[:,:], fbss_bootstrap[:,:,:] = verify.calc_score_bootstrap(reference_error, forecast_error, number_shuffle_bootstrap, box_sizes_temp)
    
    # write to fbss and errors to file
    verify.write_score_to_file(fbss, fbss_bootstrap, forecast_error, reference_error, write2file, grid, box_sizes, time_flag, filename_out, path_out)
