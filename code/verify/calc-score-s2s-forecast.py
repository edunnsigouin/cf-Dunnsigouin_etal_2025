...
Code includes possibility of calculating only high resolution data or low resolution.
...
for  i, date in enumerate(forecast_dates):
    ...
    forecast_error[i, ...], reference_error[i, ...] = verify.calc_forecast_and_reference_error(score_flag,filename_verification, filename_forecast, variable, box_sizes, grid, pval)
      
# calc score with bootstraping over all forecasts  
score[:,:], score_bootstrap[:,:,:] = verify.calc_score_bootstrap(reference_error, forecast_error, number_bootstrap, box_sizes)
...
[lead_time_gained, max_skill_mask]         = verify.initialize_ltg_and_max_skill_arrays(dim,box_sizes,number_bootstrap,dt,grid,time_flag)
lead_time_gained[:,:], max_skill_mask[:,:] = verify.calc_lead_time_gained(path_out + filename_out, dt)
verify.append_score_file(lead_time_gained, max_skill_mask, path_out + filename_out, write2file) 
