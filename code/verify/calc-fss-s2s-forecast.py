...
season                   = 'mjjas'                 # pick forecasts in specific season (djf,mam,jja,son,annual)
grids                    = ['0.25x0.25']
...
for grid in grids:
    ...
    [score,score_bootstrap] = verify.initialize_score_array(score_flag,dim,box_sizes_temp,number_shuffle_bootstrap)
    ...
    for  i, date in enumerate(forecast_dates):
        ...
        forecast_error[i, ...], reference_error[i, ...] = verify.calc_forecast_and_reference_error(score_flag,filename_verification, filename_forecast, variable, box_sizes_temp)
       
    # calc fss with bootstraping over all forecasts  
    score[:,:], score_bootstrap[:,:,:] = verify.calc_score_bootstrap(reference_error, forecast_error, number_shuffle_bootstrap, box_sizes_temp)
    
    # write to fss and errors to file
    verify.write_score_to_file(score, score_bootstrap, forecast_error, reference_error, write2file, grid, box_sizes, time_flag, filename_out, path_out)
