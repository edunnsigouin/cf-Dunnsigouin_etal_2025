...
grid                     = '0.25x0.25'
box_size                 = 41                       # smoothing box size in grid points per side. Must be odd!
lead_time                = 2
...
filename_out            = 'fss_xy_' + variable + '_' + time_flag + '_' + domain + '_' + season + '_boxsize_' + str(box_size) + '_leadtime_' + str(lead_time) + '_' + forecast_dates[0] + '_' + forecast_dates[-1] + '_' + grid + '.nc'
...
verify.write_score_to_file_xy(score, score_bootstrap, write2file, filename_out, path_out)
