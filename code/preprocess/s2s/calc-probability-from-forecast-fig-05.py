...
number_forecasts    = 5                        # number of forecast initializations 
season              = 'annual'
...
for date in forecast_dates:
    ...
    filename_in_forecast = path_in_forecast + variable + '_' + grid + '_' + date + '.nc'
    filename_in_hindcast = find_most_recent_hindcast(filename_in_forecast, path_in_hindcast) # find most recent bi-weekly hindcast!  
    filename_out         = path_out + variable + '_' + grid + '_' + date + '.nc'

    # read data 
    dim      = verify.get_data_dimensions(grid, time_flag, domain)
    forecast = xr.open_dataset(filename_in_forecast).sel(latitude=dim.latitude, longitude=dim.longitude, method='nearest')[variable]
    hindcast = xr.open_dataset(filename_in_hindcast).sel(latitude=dim.latitude, longitude=dim.longitude, method='nearest')[variable]
    ...
    if write2file: misc.to_netcdf_with_packing_and_compression(probability, filename_out)
