...
from Dunnsigouin_etal_2025  import misc,s2s
...
def resample_daily_to_weekly(ds, time_flag, grid, variable):
    ...
    def resample_days(ds,grid):
        ...
        elif ((grid == 'day1to46_0.5x0.5') and (variable == 'tp24')):
            ds_resampled = xr.concat([
    ...
    if time_flag != 'weekly':
        return ds
    elif (grid == '0.25x0.25') or (grid == '0.5x0.5') or (grid == 'day1to46_0.5x0.5'):
...
def calc_forecast_and_reference_error(score_type, filename_verification, filename_forecast, variable, box_sizes_temp, grid,pval=0.9):
    ...
    """
    # read data
    if (grid == '0.25x0.25') or (grid == 'day1to46_0.5x0.5'):
